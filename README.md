...
## What Jarvis can do right now (v1.4.3)
...
- **Workspace overview auto-refresh after approved writes (v1.4.3)** — after a workspace write is approved — either through the Workspace Files "Propose safe edit → Approve write" flow or through a chat `jarvis-write-proposal` approval — the Workspace Overview is automatically kept in sync. If the overview panel is open at the moment of approval, it re-fetches immediately and the stats, largest-file list, and recently-modified list update without any manual reload. If the overview is closed, the cached data is cleared so the next time the overview is opened it always fetches fresh metadata. Cancelling a proposal or a failed approval never triggers a refresh. A single activity event `Workspace overview refreshed after approved write` is logged only when the overview was actually visible at the time of approval.
- **Workspace overview file deep-link (v1.4.2)** — file paths listed in the Workspace Overview "Largest files" and "Recently modified" sections are now **clickable buttons**. Clicking a path switches from the overview to the file browser, navigates to the file's parent folder (if different from the current directory), and opens the file preview — exactly as if you had browsed to the file manually. The full Attach / Ask Jarvis / Propose safe edit action set is available once the preview opens. **No file is written, modified, or deleted by clicking a link.** An activity event is logged with the file path only (never content). The overview itself remains read-only.
- **Workspace overview ask helper (v1.4.1)** — the workspace Overview panel now has an **Ask Jarvis about this workspace** button. Clicking it generates a structured prompt from the overview metadata (file and folder counts, top file types, largest files, recently modified files, detected project files) and pre-fills the chat input with it. The chat view switches to the foreground automatically so the user can review and edit the question before pressing Send. **Nothing is sent automatically — the prompt is only a suggestion.** No file contents are ever included in the prompt, only workspace metadata. Activity event logged: `Workspace overview queued for chat analysis`.
- **Workspace intelligence foundation (v1.4.0, read-only)** — the Workspace Files panel now has an **Overview** button in the header. Clicking it switches to a read-only overview panel that shows: total file and folder counts; a bar chart of the top 15 file extensions; the 10 largest files with sizes; the 10 most recently modified files with dates; and project-file hints (README, package.json, tsconfig.json, Makefile detected in the workspace). Data is fetched on demand from a new `GET /files/overview` backend endpoint. The scan is capped at 2000 files and skips `node_modules`, `.git`, `.next`, `dist`, `build`, and other build artifacts. **No file contents are read or returned — metadata only. Absolute paths are never exposed.** Clicking Overview again or navigating to a file returns to the normal file browser. A safety note ("Workspace overview is read-only. It never changes files.") is always visible in the overview panel.
- **Agent plan progress summary (v1.3.5)** — the agent plan panel now shows a compact progress summary below the plan title. It displays step counts in the format `X/Y done · N active · N planned · N blocked` (zero-count categories are omitted) with colour coding (green for done, amber for active, red for blocked). A green progress bar beneath the stats shows the percentage of steps completed. The header also shows a compact `X/Y` done counter in green when work has started. All values are derived live from step statuses — no separate state — so they update immediately when the user marks a step done, resets it, or sets it active. **This is a visual-only feature; no actions are triggered by progress.**
- **Agent next-action prompt helper (v1.3.4)** — each step in the agent plan panel now has an **Ask Jarvis** link. Clicking it pre-fills the chat input with a structured prompt that includes the plan title, summary, step title, kind, status, description, and note (if any). The prompt also contains an explicit safety instruction: *"Do not write files directly. If file changes are needed, use a jarvis-write-proposal block and wait for approval."* **The message is never sent automatically** — the user can review and edit the prompt before pressing Send. No step status is changed by clicking Ask Jarvis. An activity event is logged with the step title only.
- **Agent active step indicator (v1.3.3)** — the user can manually mark any planned or blocked step as the current active step by clicking **Set active**. The active step uses `in_progress` status and is highlighted with an amber border/background. Only one step can be active at a time — marking a new step active automatically returns the previous one to `planned`. The plan panel header shows a **▶ current step title** indicator when one is active. Marking an active step **Done** clears the active indicator. **Reset** returns a done step to `planned` (not active). Active step state persists with the plan in `localStorage` and survives page refresh. **Active step is visual/manual only — it sends nothing to the model and executes nothing.**
- **Agent plan step notes (v1.3.2)** — each step in an agent plan can now carry a short manual annotation. Click **+ Add note** under any step to open an inline textarea (max 1000 chars). Click **Save note** to persist it; **Edit note** to change it; **Clear note** to remove it. Notes survive page refresh (stored as part of the plan in `localStorage`). **Notes are manual-only planning annotations — they are never injected into the Ollama prompt and never trigger any action.** Only one note editor can be open at a time.
- **Agent plan persistence per session (v1.3.1)** — agent plans now survive page refresh. Each session's active plan is saved to `localStorage` under `jarvis:agent-plan-by-session` (a map keyed by session ID). The plan is restored automatically when the chat session is reopened. Step status changes (Done / Reset) and plan dismissal are also persisted immediately. Deleting a session removes its saved plan from the map. Plans remain browser-only — no backend changes; no database writes.
- **Agent workflow foundation (v1.3.0, planning only)** — Jarvis can now present a structured step-by-step agent plan in response to multi-step requests. The local Ollama model emits a `jarvis-agent-plan` fenced block; the UI detects it and shows a plan panel with the plan title, optional summary, and ordered steps. Each step shows its kind badge (`analysis` / `code` / `docs` / `test` / `review`), a status badge (`planned` / `in_progress` / `done` / `blocked`), and **Done** / **Reset** toggle buttons. A "Planning only. Steps do not run automatically." notice is always visible. User-pasted agent plans are intercepted before Ollama — the plan panel appears without any model call. **No step runs automatically. No files are written by the plan. Write proposals still require a separate `jarvis-write-proposal` block and the normal Approve flow.** The plan resets on page refresh (foundation behaviour; persistence planned for a later milestone).
- **Per-file approve/skip for multi-file proposals (v1.2.3)** — each file in a pending multi-file write proposal now has an **✓ Include / Skip** toggle button. All files are included by default. Toggling a file to Skip dims it and removes it from the selection. The stats row updates live to show `N files · M selected · K skipped · M create · Y update · X KB selected`. The approve button changes to **Approve selected M files** and is disabled when nothing is selected, showing a "No files selected" warning. Skipped files are never passed to the write endpoint. Cancel still cancels the whole proposal. Single-file proposals are unchanged.
- **Multi-file proposal validation UI (v1.2.2)** — the pending multi-file write approval banner now shows a compact **stats row** (file count, create/update split, total content size) and **advisory warnings** for duplicate target paths and empty content. Each per-file section shows a colour-coded operation badge (**new file** in green, **update** in muted grey), the content byte size, and a `N/M` counter. Warnings are advisory only — the Approve all button is never blocked by them; backend validation remains the authoritative safety layer.
- **Multi-file proposal template helper (v1.2.1)** — a **Copy multi-file template** button below the chat input copies a ready-to-use v2 `jarvis-write-proposal` fenced JSON block to the clipboard. The template includes two example file entries (`create` + `update`) with safe workspace-relative paths. Paste it into the input, edit the paths and content, then click Send — the proposal is intercepted locally and shown in the **Pending Write Approval** banner without calling Ollama. The local Ollama model also has a short v2 format hint added to its system prompt. Approval is always required; nothing is written automatically.
- **Multi-file write proposals (v1.2.0)** — the assistant can now propose changes to multiple workspace files in a single response using the version-2 proposal format (`type: "workspace_write_proposal"`, `version: 2`, `files[]`). The approval banner shows a file-count badge, an optional summary, and a per-file diff viewer (path, operation badge, scrollable diff). A single **Approve all N files** button writes them sequentially — stopping on first failure. Backward-compatible: v1 single-file proposals (`{path, content}`) work exactly as before. Max 5 files per proposal. All existing workspace safety rules apply; nothing is written until the user clicks Approve.
